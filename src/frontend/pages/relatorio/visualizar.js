...
function renderizarRelatorio(dados) {
  ...
  if (dados.nome_completo) {
    document.getElementById('patient-nome').textContent = dados.nome_completo;
    const partesMeta = [formatarSexo(dados.sexo)];
    if (dados.data_hora) {
      partesMeta.push(`Triagem em ${formatarData(new Date(dados.data_hora))}`);
    }
